...
#     # Calculate the dot product between the normalized matrices
#     dot_product = np.dot(matrix1, matrix2.T)
    
#     # Calculate the cosine similarity
#     similarity_matrix = dot_product / (norm_matrix1.dot(norm_matrix2.T))
    
#     return similarity_matrix

# def retrieve_relevant_sents(transcript, answer):
#     sents_transcript = sentence_segmentation(transcript)
#     sents_answer = answer.split("\n")

#     vectorizer = TfidfVectorizer()
#     vectorizer.fit(sents_transcript + sents_answer)
#     tokens_transcript = vectorizer.transform(sents_transcript).toarray()
#     tokens_answer = vectorizer.transform(sents_answer).toarray()
#     simmat = cosine_similarity_matrix(tokens_transcript, tokens_answer)

def algin_transcript_tokens(result):
    """generating df_segments from transcrib result, aligning words with timestamp"""
    st = 0
    paragraph_idxs = []
    for r in result.segments_to_dicts():
        for w in r['words']:
            w_dict = {}
            w_dict['word'] = w['word']
            w_dict['probability'] = "%.2f"%w['probability']
            w_dict['st_sec'] = w['start']
            w_dict['end_sec'] = w['end']
            w_dict['st_doc'] = st
            w_dict['end_doc'] = st + len(w['word']) - 1
            paragraph_idxs.append(w_dict)
            st = st + len(w['word'])
            
    df_token = pd.DataFrame(paragraph_idxs)
    return df_token
...
def align_transcrpt_sents(transcript, df_token):
    clip_secs_st = []
    clip_secs_end = []
    transcript_sents = sentence_segmentation(transcript)
    for s in transcript_sents:
        st_sent = transcript.find(s)
        end_sent = st_sent + len(s) - 1
        st_sec = df_token.loc[(df_token['st_doc'] <= st_sent) & (df_token['end_doc'] >= st_sent)].iloc[0]['st_sec']
        end_sec = df_token.loc[(df_token['st_doc'] <= end_sent) & (df_token['end_doc'] >= end_sent)].iloc[0]['end_sec']
        clip_secs_st.append(st_sec)
        clip_secs_end.append(end_sec)
        
    df_sent = pd.DataFrame([(idx+1, sent) for idx, sent in enumerate(transcript_sents)], columns=['sent_id', 'sent'])
    df_sent['st'] = clip_secs_st
    df_sent['end'] = clip_secs_end
    return df_sent


class Api:
    def __init__(self, save_test_data=False):
        self.save_test_data = save_test_data
        self.base_dir = os.path.dirname(__file__)
        self.temp_dir = os.path.join(self.base_dir, 'temp')
        Path(self.temp_dir).mkdir(exist_ok=True, parents=True)
        self.data_dir = os.path.join(self.base_dir, "demodata")
        Path(self.data_dir).mkdir(exist_ok=True, parents=True)

        with open(os.path.join(self.base_dir, "key.txt"), "r") as f:
            self.OPENAI_API_KEY = f.read()

    def demo_transcribe(self):
        self.video_fp = os.path.join(self.data_dir, "Root Cause Analysis 150923.mp4")
        self.audio_fp = os.path.join(self.temp_dir, os.path.basename(self.video_fp+".mp3"))
        self.df_token = pd.read_pickle(os.path.join(self.data_dir, "df_token.pkl"))
        with open(os.path.join(self.data_dir, "transcript.txt"), "r") as f:
            transcript = f.read()
        return transcript
    
    def demo_ask_chatgpt_video(self):
        self.df_sent = pd.read_pickle(os.path.join(self.data_dir, "df_sent.pkl"))
        return self.df_sent.to_json(orient='records')
    
    def demo_clip_video(self):
        self.video_fp_dst = os.path.join(self.data_dir, "[Highlights] Root Cause Analysis 150923.mp4")

    def demo_ask_chatgpt_assessment(self):
        with open(os.path.join(self.temp_dir, "result_assessment.txt"), "r") as f:
            answer = f.read()
        return answer

    def open_file_dialog(self):
        assert self.window is not None, "self.window should not be None"
        file_types = ['Image Files (*.mp4;*.avi)'] # , 'All files (*.*)'
        fps = self.window.create_file_dialog(
            webview.OPEN_DIALOG, file_types=file_types # , allow_multiple=True
        )
        assert (fps is not None) and (len(fps) != 0), "No file specified!"
        self.video_fp = fps[0]

    def video2audio(self):
        self.audio_fp = os.path.join(self.temp_dir, os.path.basename(self.video_fp+".mp3"))
        video = moviepy.editor.VideoFileClip(self.video_fp)
        audio = video.audio # fps = audio.fps
        durantion = audio.duration
        audio.write_audiofile(self.audio_fp)
        estimated_time = durantion * 0.17 + 3
        assert estimated_time > 0, "Fail reading file!"
        return estimated_time
    
    def transcribe(self):
        # model = stable_whisper.load_model(os.path.join(self.base_dir, "weights", "large.pt"))
        model = stable_whisper.load_model(os.path.join(self.base_dir, "weights", "base.en.pt"))
        self.transcribe_result = model.transcribe(self.audio_fp, language="en")
        self.df_token = algin_transcript_tokens(self.transcribe_result)

        self.srt_fp = os.path.join(self.temp_dir, 'audio.srt')
        self.transcribe_result.to_srt_vtt(self.srt_fp, segment_level=True, word_level=False)
        assert self.transcribe_result.text != "", "Fail transcribing!"
        if self.save_test_data:
            self.df_token.to_pickle(os.path.join(self.data_dir, "df_token.pkl"))
            with open(os.path.join(self.data_dir, "transcript.txt"), "w") as f:
                f.write(self.transcribe_result.text)
        return self.transcribe_result.text
    
    def download_transcript(self):
        download_dir = os.path.join(Path.home(), "Downloads")
        srt_fp_dst_download = self.window.create_file_dialog(
            webview.SAVE_DIALOG, directory=download_dir, save_filename=os.path.basename(self.srt_fp)
        )
        shutil.copyfile(self.srt_fp, srt_fp_dst_download)
        if self.save_test_data:
            shutil.copyfile(self.srt_fp, os.path.join(self.data_dir, os.path.basename(self.srt_fp)))
        return srt_fp_dst_download

    def ask_chatgpt_video(self, text_prompt, transcript):
        self.df_sent = align_transcrpt_sents(transcript, self.df_token)
        candidates = [str(row['sent_id']) + ". " + row['sent'] for idx, row in self.df_sent.iterrows()]
        transcript = "\n".join(candidates)
        example1, example2, example3 = candidates[0], candidates[1], candidates[-1]

        text_prompt += "\n\nExample output:\n{example1}\n{example2}\n{example3}"
        prompt_template = PromptTemplate(
            input_variables=["transcript", "example1", "example2", "example3"],
            template=text_prompt
        )
        question = prompt_template.format(transcript=transcript, example1=example1, example2=example2, example3=example3)
        llm = OpenAI(temperature=.2, model_name="gpt-3.5-turbo", openai_api_key=self.OPENAI_API_KEY)
        # llm = OpenAI(temperature=.2, model_name="gpt-4", openai_api_key=self.OPENAI_API_KEY)
        ...
        for sent in answer.split("\n"):
            match = re.match("^\d+?\.", sent.strip())
            if match:
                sent_id = int(match.group(0).replace(".", ""))
                sent_ids.append(sent_id)

        self.df_sent['selected'] = False
        self.df_sent.loc[self.df_sent['sent_id'].isin(sent_ids), 'selected'] = True
        if self.save_test_data:
            self.df_sent.to_pickle(os.path.join(self.data_dir, "df_sent.pkl"))
        return self.df_sent.to_json(orient='records')
        
    def clip_video(self, selected_idxs):
        movie = mp.VideoFileClip(self.video_fp)
        filter_mask = self.df_sent['sent_id'].isin(selected_idxs)
        clip_secs = self.df_sent.loc[filter_mask, ["st", "end"]].values
            
        clips = []
        fade_duration = 0.5
        for idx, (start, end) in enumerate(clip_secs):
        ...
        self.video_fp_dst = os.path.join(self.temp_dir, "[Highlights] " + os.path.basename(self.video_fp))
        final_clip = mp.concatenate_videoclips(clips)
        final_clip.write_videofile(self.video_fp_dst)

    def download_video(self):
        download_dir = os.path.join(Path.home(), "Downloads")
        video_fp_dst_download = self.window.create_file_dialog(
            webview.SAVE_DIALOG, directory=download_dir, save_filename=os.path.basename(self.video_fp_dst)
        )
        shutil.copyfile(self.video_fp_dst, video_fp_dst_download)
        if self.save_test_data:
            shutil.copyfile(self.video_fp_dst, os.path.join(self.data_dir, os.path.basename(self.video_fp_dst)))
        return video_fp_dst_download

    def ask_chatgpt_assessment(self, text_prompt, transcript):
        prompt_template = PromptTemplate(
